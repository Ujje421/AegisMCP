    ...
    async def __call__(self, inputs: Any, ctx: AegisContext, descriptor: ToolDescriptor) -> Any:
        ...
        if now >= ctx.deadline:
            raise ToolTimeoutError(
                f"Deadline exceeded before starting tool {descriptor.name}", ctx.request_id
            )
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(descriptor.fn):
                ...
                return await asyncio.wait_for(
                    loop.run_in_executor(None, lambda: descriptor.fn(**kwargs)), timeout=timeout
                )
        except TimeoutError:
            raise ToolTimeoutError(
                f"Tool {descriptor.name} timed out after {timeout}s", ctx.request_id
            )
